...
8. The roadmap is `assets/roadmap.py`; `assets/roadmap.svg` is generated from it. `assets/tui.svg` is generated by `assets/tui.py` from a live `cones tui`; regenerate it when the dashboard layout changes. A commit that ships a roadmap item moves that item into `SHIPPED` and regenerates the SVG in the same commit.
