...
| Title | Transcript `custom-title` or user-set `agent-name`, then `ai-title`; otherwise background job `name`, registry `name`, then first instruction line. Bare job/session ids do not count as names. Dashboard rename uses native `/rename`; a manual name for an external interactive client appends `custom-title`. | `threads.name`, then `threads.title` in `state_*.sqlite`, then legacy `session_index.jsonl`'s `thread_name`, then first rollout `UserMessage`. Database reads use `sqlite3`. | Last `session_info.name`, otherwise first instruction line. |
