...
| `cones tui [--debug]` | The dashboard. `--debug` appends terminal hand-offs, input events, command durations, refresh reads and discarded snapshots, and transition-to-draw timings to `STATE_DIR/tui-debug.log`. It also samples tty modes and foreground ownership at 50ms, process states once a second, and records frames taking at least 16ms. A child that restores a cooked tty and lingers before exiting shows as the gap between its `tty:` line and `child exited`. |
