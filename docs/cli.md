...
| `cones tui [--debug]` | The dashboard. `--debug` appends every terminal hand-off (command, child pid, the instant it exits and its status, signal dispositions, every change of the tty modes and foreground owner sampled at 50ms, the process groups' states once a second) and every input event to `STATE_DIR/tui-debug.log`, for the attach-and-return bugs. A child that restores a cooked tty and lingers before exiting shows as the gap between its `tty:` line and `child exited`. |
