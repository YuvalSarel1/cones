...
| Claude interactive | registry entry, `kind: interactive`; a `claude` typed in a terminal | sessions table, footer says `own terminal` | refused: "runs in its own terminal and cannot be joined from here"; `cones attach` refuses with the same reason while the pid is alive | n/a | SIGTERM on the registry pid, after `ps` confirms the pid still runs a `claude` binary | Claude removes the entry, or the pid is gone or reused |
...
| Codex TUI | the process table: a live `codex` whose first argument is not a session-less subcommand | sessions table, footer says `own terminal` | refused, same message | n/a | SIGTERM on the pid, after `ps` confirms it still runs a `codex` binary | the process exits |
...
| Codex threads from other clients | the VS Code extension, another terminal's `--remote` | no; the daemon has no thread list yet | | | | |
