...
`operations.launch.model`, `provider` and `effort` name native flags. Their values come from the selected harness's configuration: Claude uses `defaults.model`, Codex uses `defaults.codex_model`, pi uses `defaults.pi_model` and `defaults.pi_provider`, and OpenCode uses `defaults.opencode_model`. Effort comes from `defaults.effort` for Claude and `defaults.pi_thinking` for pi; Codex and OpenCode define no effort flag. Unset values pass no flag. Claude's Bedrock environment and Codex's daemon provider remain native adapter behavior; a pi provider binding cannot be claimed for those adapters.
