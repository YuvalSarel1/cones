...
| `operations` | Launch, attach, resume, fork, stop, remove, unarchive, message delivery, rename availability and session-kind behavior. |
...
The separate `operations.stop` supplies the native command used by `cones stop` for daemon-owned sessions, preserving their conversation. It takes `{id}` or `{short_id}`. `operations.message` supplies native delivery for `cones comms send`, using `{id}`, `{text}` and optionally `{remote}`. A harness missing either operation cannot perform that native action through the CLI. Owned-terminal stopping uses the terminal host independently.

Command templates are argument arrays, and a placeholder occupies a whole argument. Accepted placeholders depend on the operation: `prompt`, `remote`, `cwd`, `id`, `short_id`, `transcript`, the fork-specific `new_id` and the message-specific `text`. Unknown placeholders and missing operands are errors. Substitution preserves spaces, newlines and native path bytes and performs no shell expansion. Positional launch prompts follow `--`. A declared `prompt_flag` sends one `--flag=value` operand, and `stdin_prompt` supplies an anonymous input file. OpenCode keeps its native `--prompt=<instruction>` form because its positional argument is a project directory. See [terminal definitions](#terminal-only-definitions-and-forks).
