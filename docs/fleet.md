...
| `enter`, `→`, `a` | On a job: start a run in the background. On a running run: follow its log (Ctrl+C returns). On a finished run or a session: open it in this terminal, as described above; Ctrl+Z comes back to the same row, with the filter and grouping as they were. On a session of a harness that cannot be joined from here (a Codex running in its own terminal), the footer says `own terminal` and `enter` explains in the status line. |
