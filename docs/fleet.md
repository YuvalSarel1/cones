...
| `enter`, `→`, `a` | On a job: start a run in the background. On a running run: follow its log (Ctrl+C returns). On a finished run or a session: open it in this terminal, as described above; Ctrl+Z comes back to the same row, with the filter and grouping as they were. On a Codex thread the dashboard launched (kind `daemon`): open a resume client on it. On a Codex TUI, or an interactive Claude, running in its own terminal, the footer says `own terminal` and `enter` explains in the status line. |
...
| `x` twice (or `ctrl+x` twice) within two seconds | Stop the selected run or session. On a Codex daemon thread: forget its record, `codex resume` still has it. On a job: stop that job's run in flight; with none, the status line says so. |
...
| `start ›` | `interactive` or `managed` (`←` `→`, or `i` / `m`). Interactive suspends the dashboard and runs the harness natively in the directory, with the harness's own permission prompts. Claude starts as a background session (`claude --bg`) with `claude attach` on it, so leaving the viewer keeps the session in the fleet and `enter` on its row opens it again. Codex starts as a thread of its app-server daemon with the TUI as a client, as described under [Codex behind its daemon](#codex-behind-its-daemon-the-rows-the-dashboard-can-open); a Codex build without the daemon is refused here with the reason in the status line. `cones doctor` reports both probes. Managed is a supervised one-off run under the read-only defaults or the first job's policy, so a harness with no adapter (Codex today) is refused here with the validation message rather than as a failed row. |
