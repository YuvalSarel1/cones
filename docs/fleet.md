...
| `ctrl+x` twice within two seconds | Stop the selected run or session. On a Claude background session: `claude rm`, so the record leaves `claude agents` too and `claude --resume` still has the conversation. On a finished run: delete it, records, output and archived transcript together. On a Codex daemon thread: forget its record, `codex resume` still has it. On a job: stop its run in flight; with none, delete the job from jobs.yaml and reinstall launchd. `esc` after the first press keeps it. |
