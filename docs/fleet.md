...
| `enter`, `→`, `a` | On a job: start a run in the background. On a running run: follow its log (Ctrl+C returns). On a finished run or a session: open it in this terminal, as described above; Ctrl+Z comes back to the same row, with the filter and grouping as they were. On a `suspended` harness: bring it back to the terminal. |
...
| `r` | Reload now. |
| `esc`, `q`, `ctrl+c` | Quit. With a suspended harness parked, the first press names it and the second kills it and quits. |

Ctrl+Z never suspends the dashboard. Every child the dashboard hands the terminal to runs in its own process group and owns the tty, like a shell job, so Ctrl+C and Ctrl+Z reach it and whatever it forked, and nothing else. What Ctrl+Z then means depends on the child. Inside `claude attach` it drops the viewer and the session keeps running. A viewer that stops instead (Claude's agents view lets Ctrl+Z through as SIGTSTP) is killed on the spot, since it has already given the terminal back and the session it showed is untouched, so the dashboard is back at once either way. A Codex launched with `n` is the session itself: Ctrl+Z parks it, stopped in its own process group, and the dashboard is back at once with the Codex as a `suspended` row (▽) in its directory; `enter` on that row resumes it where it was, Ctrl+Z parks it again. Parked means stopped, so it does not work while you are away. Quitting with a Codex parked asks twice, then kills it, since nothing could resume it afterwards. The `e` editor stops on Ctrl+Z too and is resumed at once, so Ctrl+Z is a no-op there and unsaved edits are safe. Opening a session runs `claude attach <short id>` from the dashboard itself; a finished run goes through `cones attach`, which resumes it. Before the hand-off the dashboard paints its last frame on the normal screen and erases it on the way back, so the shell never shows while Claude starts or shuts down. The dashboard's state waits in memory meanwhile and on return it reloads and finds the row again by its id, so a session that went from idle to working while it was open, and so moved to another group, is still the selected row. A session that ended while open leaves the cursor on its neighbor. Runs the dashboard starts are ordinary `cones run` subprocesses and appear in the ledger.
