...
| `ctrl+x` twice within two seconds | Stop the selected run or session. On a Codex daemon thread: forget its record, `codex resume` still has it. On a job: stop its run in flight; with none, delete the job from jobs.yaml and reinstall launchd. `esc` after the first press keeps it. |
